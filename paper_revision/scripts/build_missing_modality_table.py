...
def main() -> None:
    ...
    if df.empty or "modality_setting" not in df:
        pd.DataFrame().to_csv(args.output_dir / "missing_modality_robustness_metrics.csv", index=False)
        return
    thresholds = locked_thresholds(df)
    df = df[df["modality_setting"].fillna("none").astype(str).ne("none")].copy()
    rows = []
    for (method, run_id, seed, split, setting), group in df.groupby(["method", "run_id", "seed", "split", "modality_setting"], dropna=False):
    ...
    out.to_csv(args.output_dir / "missing_modality_robustness_metrics.csv", index=False, encoding="utf-8-sig")
    display = out.copy()
    ...
    display.to_csv(args.output_dir / "missing_modality_robustness_metrics_formatted.csv", index=False, encoding="utf-8-sig")
    print(f"Wrote {len(out)} missing-modality rows")
